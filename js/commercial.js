...
const CONTACT_FORM = document.getElementById('quote.html');
...
CONTACT_FIELDS.forEach(field => {
	field.addEventListener('remove', () => field.classList.remove('is-invalid'));
});
...
CONTACT_FORM.addEventListener("submit", async (e) => {
	...
	try {
		...
		let RESPONSE = await fetch(CONTACT_FORM_URL, {
			method: "POST",
			headers: {
